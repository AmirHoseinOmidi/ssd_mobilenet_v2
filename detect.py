...
class ObjectDetection:
    def __init__(self, model_dir, class_names, video_source=0):
    ...
    def process_frame(self, frame):
        ...
        output_dict = self.run_model(image_resized)
        ...
        if len(boxes) > 0:
            ...
            for i in range(len(boxes)):
                if scores[i] > 0.5:
                    ...
                    self.detected_objects.append(f"{class_name}: {scores[i]:.3f}")
    
    def display_fps(self, frame, start_time):
    ...
    def start(self):
        while True:
            ...
            self.display_fps(frame, start_time)
...
detector = ObjectDetection(model_dir, class_names)
